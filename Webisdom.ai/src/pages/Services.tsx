...
const Services = () => {
  ...
  // --- FIX: Scroll to top when opening/closing a blog ---
  useEffect(() => {
      ...
      <div className="min-h-screen bg-white text-slate-900 selection:bg-blue-100">
        ...
        <main className="pt-32 pb-20 relative z-10">
          <div className="container mx-auto px-4 max-w-4xl">
            <Button 
              variant="ghost" 
              className="mb-8 text-slate-500 hover:text-blue-600"
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <div 
                ...
                prose-headings:text-slate-900 prose-blockquote:border-blue-500 prose-blockquote:bg-blue-50 prose-blockquote:rounded-xl prose-blockquote:py-1"
                dangerouslySetInnerHTML={{ __html: selectedBlog.content }}
              />

              <div className="mt-20 p-10 bg-slate-50 border border-slate-200 rounded-3xl text-center">
                <h3 className="text-2xl font-bold mb-4 text-slate-900">Want to implement this technology?</h3>
                <p className="text-slate-500 mb-8">Our AI Lab specializes in custom {selectedBlog.category} deployments.</p>
                <Button size="lg" className="rounded-full px-10 bg-blue-600 hover:bg-blue-700" onClick={() => setIsDemoDialogOpen(true)}>
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 selection:bg-blue-200 relative overflow-x-hidden">
      ...
      <main className="relative">
        ...
        <section className="relative pt-48 pb-44 text-center overflow-hidden min-h-[80vh] flex items-center bg-[#020617]">
          <div className="absolute inset-0 z-0">
             ...
             <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(2,6,23,0.3)_0%,rgba(2,6,23,0.95)_100%)] pointer-events-none" />
          ...
          <div className="container mx-auto px-4 max-w-5xl relative z-10">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
              <Badge variant="outline" className="mb-8 border-blue-400/40 text-blue-400 uppercase tracking-widest py-2.5 px-8 bg-blue-500/10 backdrop-blur-xl">
                <span className="flex items-center font-bold"><Sparkles className="w-4 h-4 mr-2" /> Expertise & Innovation</span>
              </Badge>
              
              <h1 className="text-6xl md:text-9xl font-extrabold mb-8 tracking-tighter text-white">
                Enterprise <span className="text-blue-500">AI Solutions</span>
              </h1>
              
              <p className="text-xl md:text-3xl text-blue-50/80 max-w-3xl mx-auto leading-relaxed font-medium mb-12">
                Empowering global businesses with high-fidelity auditing and strategic neural automation.
              </p>
              
              <div className="flex flex-wrap justify-center gap-6">
                <Button size="lg" className="h-16 px-12 text-lg rounded-full bg-blue-600 hover:bg-blue-700 text-white shadow-2xl transition-all" onClick={() => setIsDemoDialogOpen(true)}>
                  Schedule a Demo <ChevronRight className="ml-2 w-5 h-5" />
                </Button>
                <Button size="lg" variant="outline" className="h-16 px-12 text-lg rounded-full border-white/40 text-white bg-white/5 hover:bg-white/10 backdrop-blur-md transition-all" onClick={() => setIsAuditDialogOpen(true)}>
                  Request AI Audit
                </Button>
              </div>
            </motion.div>
        ...
        <section className="py-24 relative z-10 -mt-20" id="ai-audit">
        ...
        <section className="py-24 relative z-10" id="blog">
          <div className="container mx-auto px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
              {blogs.map((blog) => (
                <div
                  key={blog.id}
                  className="bg-white border border-slate-200 rounded-[2rem] shadow-lg hover:shadow-2xl hover:border-blue-400 transition-all duration-500 overflow-hidden group cursor-pointer"
                ...
                >
                  ...
                  <div className="p-10 space-y-5">
                    ...
                    <div className="pt-6 border-t border-slate-100 flex items-center text-blue-600 text-sm font-bold uppercase tracking-widest gap-2">
                      Full Analysis <ChevronRight className="h-4 w-4 group-hover:translate-x-2 transition-transform" />
                    </div>
                  </div>
                </div>
              ))}
